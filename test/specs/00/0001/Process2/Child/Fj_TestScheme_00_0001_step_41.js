...
export default async function suite() {
  it("Fj_TestScheme_00_0001_step_41: An 申込 application is made", async () => {
    ...
    await browser.url(
      user_info.userInformation.var_sf_sfurl +
        test_data.testData.httpurl09 +
        test_data.testData.listview6_id
    );
    ...
    while (sortmu !== test_data.testData.app_desc) {
      await controlno.click();
      await browser.pause(5000);
      sortmu = await controlno.nextElement().getText();
    }
    await expect(sortmu).toBe(test_data.testData.app_desc);
    ...
    while (sortapp !== test_data.testData.app_desc) {
      await appid.click();
      await browser.pause(5000);
      sortapp = await appid.nextElement().getText();
    }
    await expect(sortapp).toBe(test_data.testData.app_desc);
    ...
    while (sortacc !== test_data.testData.app_desc) {
      await acc_date.click();
      await browser.pause(5000);
      sortacc = await acc_date.nextElement().getText();
    }
    await expect(sortacc).toBe(test_data.testData.app_desc);
    ...
    while (sortcon !== test_data.testData.app_desc) {
      await con_date.click();
      await browser.pause(5000);
      sortcon = await con_date.nextElement().getText();
    }
    await expect(sortcon).toBe(test_data.testData.app_desc);
