...
export default async function suite() {
  it("Fj_TestScheme_00_0008_step_09: The 子画面sub screen is displayed and the confirmation operation is possible", async () => {
    ...
    const results_val = await $(
      "//span[@title='" +
        test_data.testData.asc_antisocial_results_value2 +
        "']"
    ); // ★ 新環境適用' New Env Implementation
    ...
    // ★ 新環境適用' New Env Implementation
    // Click 確認
    const approvebtn = await $(
      ".//button[@name='" + test_data.testData.asc_accept_btn_api + "']"
    );
    await approvebtn.waitForClickable({ timeout: 5000 });
