...
export default function suite() {
  it("Fj_TestScheme_00_0006_step_24: The rows that you have added appear in the table at the top of the 借入明細情報一覧画面Borrowed Line Information Listing screen as an 「追加」区分Additional category", async () => {
    ...
    await $(".//input[@id='" + loan_amount_id + "']").setValue(
      test_data.testData.trr_loan_amount_val
    );
    await browser.pause(1000);
    ...
    await $(".//input[@id='" + annual_repayment_id + "']").setValue(
      test_data.testData.trr_annual_repayment_val
    );
    await browser.pause(1000);
    ...
    await $(
      ".//lightning-base-combobox-item[contains(.,'" +
        test_data.testData.trr_collateral_classify_val +
        "')]"
    ).click();
    await browser.pause(1000);
