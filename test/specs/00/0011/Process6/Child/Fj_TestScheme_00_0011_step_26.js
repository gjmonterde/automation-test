...
export default function suite() {
  it("Fj_TestScheme_00_0011_step_26: The confirmation screen is displayed and the input value is reflected", async () => {
    ...
    if (
      test_data.testData.mypage_duration_value_26 === "" ||
      test_data.testData.mypage_duration_value_26 >= inp2
    ) {
      test_data.testData.mypage_duration_value_26 = Math.floor(inp2 / 2);
    }
    await input2.setValue(test_data.testData.mypage_duration_value_26);

    if (await $(".//select[@id='page:pb1:form1:input3']").isExisting()) {
      // 毎月の返済日
      await $(".//select[@id='page:pb1:form1:input3']").selectByAttribute(
        "value",
        test_data.testData.mypage_repayment_date_value_26
      );
    }
    ...
    if (await $(".//input[@id='page:pb1:form1:useBonus']").isExisting()) {
      ...
      if (
        ...
        test_data.testData.mypage_loan_bonus_use_toggle_26 ==
      ...
      await $(".//select[@id='page:pb1:form1:bonusOne']").selectByAttribute(
        "value",
        test_data.testData.mypage_repayment_month_value_26
      );

      // 増額返済割合
      await $(".//select[@id='page:pb1:form1:bonusTwo']").selectByAttribute(
        "value",
        test_data.testData.mypage_repayment_ratio_value_26
      );
    }
    ...
    await $(".//input[@id='page:pb1:form1:datePicker']").setValue(
      test_data.testData.mypage_loan_date_value_26
    );
