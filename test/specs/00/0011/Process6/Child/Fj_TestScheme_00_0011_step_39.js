...
export default function suite() {
  it("Fj_TestScheme_00_0011_step_39: The confirmation screen is displayed and the input value is reflected", async () => {
    ...
    if (
      test_data.testData.mypage_borrow_amount_value_39 === "" ||
      test_data.testData.mypage_borrow_amount_value_39 >= inp1
    ) {
      test_data.testData.mypage_borrow_amount_value_39 = Math.floor(inp1 / 2);
    }
    await input1.setValue(test_data.testData.mypage_borrow_amount_value_39);

    // 毎月の返済日
    if (await $(".//select[@id='page:pb1:form1:input3']").isExisting()) {
      await $(".//select[@id='page:pb1:form1:input3']").selectByAttribute(
        "value",
        test_data.testData.mypage_repayment_date_value_39
      );
    }
    ...
    if (await $(".//input[@id='page:pb1:form1:useBonus']").isExisting()) {
      ...
      if (
        ...
        test_data.testData.mypage_loan_bonus_use_toggle_39 ==
      ...
      await $(".//select[@id='page:pb1:form1:bonusOne']").selectByAttribute(
        "value",
        test_data.testData.mypage_repayment_month_value_39
      );

      // 増額返済割合
      await $(".//select[@id='page:pb1:form1:bonusTwo']").selectByAttribute(
        "value",
        test_data.testData.mypage_repayment_ratio_value_39
      );
    }
    ...
    await $(".//input[@id='page:pb1:form1:datePicker']").setValue(
      test_data.testData.mypage_loan_date_value_39
    );
