...
export default async function suite() {
  it("Fj_TestScheme_00_0011_step_31: The返済シミュレーション画面 repayment simulation screen is deactivated", async () => {
    ...
    if (
      test_data.testData.app_borrow_amount_value_31 === "" ||
      test_data.testData.app_borrow_amount_value_31 >= inp1
    ) {
      test_data.testData.app_borrow_amount_value_31 = Math.floor(inp1 / 2);
    }
    await input1.setValue(test_data.testData.app_borrow_amount_value_31);

    // Input date
    await $(".//input[@name='page:pb1:form1:datePicker']").setValue(
      test_data.testData.app_preferred_loan_date_value_31
    );
