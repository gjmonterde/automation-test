...
export default function suite() {
  it("Fj_TestScheme_00_0011_step_41: The お申込みステータス application status on My Page must be 「ご融資条件ご確定」Finalized Loan Terms", async () => {
    ...
    if ((await browser.getUrl()) === test_data.testData.mypage_url) {
      ...
      if (test_data.testData.mypage_borrow_amount_value_41 != "") {
        const input1 = await $(".//input[@id='page:pb1:form1:input1']");
        await input1.click();
        await browser.execute((val) => {
          val.value = null;
        }, input1);
        await input1.setValue(test_data.testData.mypage_borrow_amount_value_41);
      }

      // 期間
      // NOTE: change 期間 value in test_info
      if (test_data.testData.mypage_duration_value_41 != "") {
        const input2 = await $(".//input[@id='page:pb1:form1:input2']");
        await input2.click();
        await browser.execute((val) => {
          val.value = null;
        }, input2);
        await input2.setValue(test_data.testData.mypage_duration_value_41);
      }
      ...
      if (await $(".//select[@id='page:pb1:form1:input3']").isExisting()) {
        ...
        await $(".//select[@id='page:pb1:form1:input3']").selectByAttribute(
          "value",
          test_data.testData.mypage_repayment_date_value_41
        );
      }
      ...
      if (await $(".//input[@id='page:pb1:form1:useBonus']").isExisting()) {
        ...
        if (
          ...
          test_data.testData.mypage_loan_bonus_use_toggle_41 ==
        ...
        await $(".//select[@id='page:pb1:form1:bonusOne']").selectByAttribute(
          "value",
          test_data.testData.mypage_repayment_month_value_41
        );

        // 増額返済割合
        await $(".//select[@id='page:pb1:form1:bonusTwo']").selectByAttribute(
          "value",
          test_data.testData.mypage_repayment_ratio_value_41
        );
      }
      ...
      await $(".//input[@id='page:pb1:form1:datePicker']").setValue(
        test_data.testData.mypage_loan_date_value_41
      );
