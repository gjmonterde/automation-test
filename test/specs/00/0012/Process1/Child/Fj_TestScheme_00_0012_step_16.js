...
export default function suite() {
  it(
    "Fj_TestScheme_00_0012_step_16: 実行依頼データ execution Submit data of record type「22_証書貸付_回収」" +
      '"22_Loan_Collection of deeds" is created according to the number of 貸出共通明細（既貸回収flag＝TRUE　and　科目コード＝44）',
    async () => {
      ...
      // Get 14_カードローン回収 records
      await parent.Get_ER_Array_Record(test_data.testData.er_14_rectype); // ★ 新環境適用' New Env Implementation

      // Screenshot variable
      var ssno = 1; // ★ 新環境適用' New Env Implementation

      if (test_data.testData.er_array > 0) {
      ...
      await parent.Get_LCD_Record_collect(
        test_data.testData.lcd_subject_code_44
      );
