...
export default function suite() {
  it("Fj_TestScheme_00_0012_step_08: 実行依頼データ Submit data for the record type「14_カードローン回収」has been created", async () => {
    ...
    await parent.Get_ER_Array_Record(test_data.testData.er_14_rectype);
    ...
    // Get LCD records
    await parent.Get_LCD_Record_collect(test_data.testData.lcd_subject_code_1);
    ...
    if (test_data.testData.lcd_id_arr.length > 0) {
      ...
      for (var lcd = 0; lcd < test_data.testData.lcd_id_arr.length; lcd++) {
        // Go to LCD record
        await parent.Open_LCD_Record(test_data.testData.lcd_id_arr[i]);
