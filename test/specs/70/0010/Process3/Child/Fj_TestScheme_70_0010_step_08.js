...
export default async function suite() {
  it("Fj_TestScheme_70_0010_step_08: Requested document status is Not Submitted", async () => {
    ...
    // Get the 1st RDC record NOTE: Please change if necessary
    // ★ 新環境適用' New Env Implementation
    let rdc_no = 0;

    // Go to RDC page
    await parent.Go_To_RDC(test_data.testData.rdc_id_arr[rdc_no]); // ★ 新環境適用' New Env Implementation
    ...
    const rdc_reject_radio_btn = await $(
      "//label[@for=//input[@name='" +
        test_data.testData.rdc_id_arr[rdc_no] +
    ...
    const rdc_defect_reason_val = await $(
      "//option[@value='" + test_data.testData.rdc_defect_reason_val + "']"
    );
    let rdc_defect_reason_val_clickable =
      await rdc_defect_reason_val.isClickable();
    console.log(
      "rdc_defect_reason_val is clickable: " + rdc_defect_reason_val_clickable
    );
    await rdc_defect_reason_val.click();
    await browser.pause(5000);
