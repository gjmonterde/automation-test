...
export default async function suite() {
  it("Fj_TestScheme_10_0001_step_07/11/12/14/17/18/20/21: Application Record Creation", async () => {
    ...
    await $("#step1_submit").click();
    await $(
      ".//input[@value='" + test_data.testData.back_btn_form + "']"
    ).scrollIntoView();
    await $(
      ".//input[@value='" + test_data.testData.back_btn_form + "']"
    ).click();
    ...
    await $(
      ".//*[contains(text(), '" + test_data.testData.app_success_msg + "')]"
    ).waitForExist({ timeout: 100000 });
