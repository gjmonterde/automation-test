...
export default async function suite() {
  it("Fj_TestScheme_10_0011_step_12: The 銀行口座 bank account registered on My Page should appear in the relevant list of applications", async () => {
    ...
    // 確認ステータス
    await $("label=" + test_data.testData.ba_status_edit_title).$(function () {
      this.scrollIntoView({
        behavior: "smooth",
        block: "center",
        inline: "center",
    ...
    const status_id = await $(
      "label=" + test_data.testData.ba_status_edit_title
    ).getAttribute("for");
    ...
    // Screenshot
    const highlights12_4 = await $(
      ".//div[@class='highlights slds-clearfix slds-page-header slds-page-header_record-home fixed-position']"
    );
    const headerBar12_4 = await $(".//header[@id='oneHeader']");
    const tabheader12_4 = await $(
      ".//div[@class='slds-no-print oneAppNavContainer']"
    );
    await browser.saveFullPageScreen(
      // ★ 新環境適用' New Env Implementation
      user_info.userInformation.screenshot +
        ...
        "-4",
      {
        hideAfterFirstScroll: [headerBar12_4, tabheader12_4, highlights12_4],
        fullPageScrollTimeout: 1000,
      }
    );
  });
