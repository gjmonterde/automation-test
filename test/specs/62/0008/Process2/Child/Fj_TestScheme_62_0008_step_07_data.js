...
const parent = require("../Parent/Fj_TestScheme_62_0008-2");

export default async function suite() {
  it("Fj_TestScheme_62_0008_step_07_data: Manual process for 反社照会 record field update (data linkage)", async () => {
    ...
    // 授受結果
    const giving_lbl = await $(
      "//label[contains(., '" + test_data.testData.asc_giving_result_lbl + "')]"
    );
    await giving_lbl.$(function () {
      this.scrollIntoView({
        behavior: "smooth",
        block: "center",
        inline: "center",
    ...
    await giving_lbl.waitForClickable({ timeout: 5000 });
    await giving_lbl.click();
    await browser.pause(2000);
    const giving_val = await $(
      "//span[@title='" + test_data.testData.asc_giving_result_value + "']"
    );
    await giving_val.scrollIntoView({ block: "center" });
    await giving_val.waitForClickable({ timeout: 5000 });
    await giving_val.click();
    await browser.pause(2000);
    ...
    await status_val.scrollIntoView({ block: "center" });
