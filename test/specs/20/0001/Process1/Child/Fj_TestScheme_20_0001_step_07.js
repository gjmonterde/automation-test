...
const parent = require("../Parent/Fj_TestScheme_20_0001-1");

export default async function suite() {
  it("Fj_TestScheme_20_0001_step_07/11/12/14/16/18/20/21: Application record creation", async () => {
    ...
    if ((await browser.getUrl()) != test_data.testData.httpurl01) {
      await parent.Go_to_app_form();
    }
    // 7
    // Click Submit button
    ...
    const acc_1 = await $(
      ".//dl[@class[contains(., '" + test_data.testData.acc6 + "')]]"
    );
    ...
    const acc_2 = await $(
      ".//dl[@class[contains(., '" + test_data.testData.acc7 + "')]]"
    );
    ...
    // 契約規定・保証委託約款
    const acc_3 = await $(
      ".//dl[@class[contains(., '" + test_data.testData.acc8 + "')]]"
    );
