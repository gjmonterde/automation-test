...
const parent = require("../Parent/Fj_TestScheme_20_0002-3");
const util = require("../../../../../pageobjects/utility.js");

export default function suite() {
  it("Fj_TestScheme_20_0002_step_15: Should be able to select value from 不備理由 field for 徴求書類 record", async () => {
    ...
    const defect_reason_val = await $(
      "//option[@value='" + test_data.testData.defect_reason_val + "']"
    );
    let defect_reason_val_clickable = await defect_reason_val.isClickable();
    console.log(
      "defect_reason_val is clickable: " + defect_reason_val_clickable
    );
    await defect_reason_val.click();
    await browser.pause(5000);

    // Input value in メモ field
    const memo_txtinput = await $(
      "//input[@name='" +
        test_data.testData.rdc1_id +
        test_data.testData.rdc_memo_input +
        "']"
    );

    let memo_txtinput_clickable = await memo_txtinput.isClickable();
    console.log("memo_txtinput is clickable: " + memo_txtinput_clickable);

    await memo_txtinput.setValue(test_data.testData.memo_val);
    await browser.pause(5000);
