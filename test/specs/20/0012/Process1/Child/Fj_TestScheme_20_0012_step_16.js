...
export default async function suite() {
  it("Fj_TestScheme_20_0012_step_16: Submit data of record type「21_カードローン口座閉鎖」 is created acoording to the number of  貸出共通明細（既貸回収flag＝TRUE　and　科目コード＝48", async () => {
    ...
    var ssno = 2;
    // ★ 新環境適用' New Env Implementation
    if (test_data.testData.lcd48_id_arr.length > 0) {
      for (var i = 0; i < test_data.testData.lcd48_id_arr.length; i++) {
        // Go to LCD record
        await parent.Go_to_LCD(test_data.testData.lcd48_id_arr[i]);
