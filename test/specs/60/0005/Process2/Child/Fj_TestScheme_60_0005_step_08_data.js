...
export default function suite() {
  it("Fj_TestScheme_60_0005_step_08_data: KSC結果(CIC), KSC結果(JICC), KSC結果(取引情報), KSC結果(本人申告) Data Linkage", async () => {
    // Login as Admin
...
async function KIT_Record_Creation() {
  ...
  for (var i = 0; i < test_data.testData.three_no_of_records; i++) {
    ...
    const contract_date = await $(
      ".//records-record-layout-item[@field-label='" +
        test_data.testData.contractdate_lbl +
        "']"
    ).$(".//label[contains(.,'" + test_data.testData.contractdate_lbl + "')]");
    ...
    const last_repayment_date = await $(
      ".//records-record-layout-item[@field-label='" +
        test_data.testData.last_repayment_lbl +
        "']"
    ).$(
      ".//label[contains(.,'" + test_data.testData.last_repayment_lbl + "')]"
    );
    ...
    const completion = await $(
      ".//records-record-layout-item[@field-label='" +
        test_data.testData.completiontype_lbl +
        "']"
    ).$(
      ".//label[contains(.,'" + test_data.testData.completiontype_lbl + "')]"
    );
    ...
    if (i == 0) {
      ...
      const loan_amount_card = await $(
        ".//records-record-layout-item[@field-label='" +
          test_data.testData.loanamt_card_lbl +
          "']"
      ).$(
        ".//label[contains(.,'" + test_data.testData.loanamt_card_lbl + "')]"
      );
...
async function KIL_Record_Creation() {
  ...
  const attribchangedate = await $(
    ".//records-record-layout-item[@field-label='" +
      test_data.testData.attrib_changedate_lbl +
      "']"
  ).$(
    ".//label[contains(.,'" + test_data.testData.attrib_changedate_lbl + "')]"
  );
  ...
  const declare_category = await $(
    ".//records-record-layout-item[@field-label='" +
      test_data.testData.declare_category_lbl +
      "']"
  ).$(
    ".//label[contains(.,'" + test_data.testData.declare_category_lbl + "')]"
  );
  ...
  const declare_date = await $(
    ".//records-record-layout-item[@field-label='" +
      test_data.testData.declare_date_lbl +
      "']"
  ).$(".//label[contains(.,'" + test_data.testData.declare_date_lbl + "')]");
