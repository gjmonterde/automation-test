...
export default async function suite() {
  it("Fj_TestScheme_63-1_0011_step_11: Check the registered 銀行口座 bank account is displayed in the 振込先一覧", async () => {
    ...
    for (var i = 0; i < test_data.testData.ba_new_record_no_value; i++) {
      ...
      // 金融機関名
      if (i == 1) {
        const financial_institution_name_label = await $(
          "label=" + test_data.testData.financial_institution_name_label
        ).getAttribute("for");
        await $(
          ".//input[@id='" + financial_institution_name_label + "']"
        ).setValue(
          eval("test_data.testData.financial_institution_name_value" + (i + 1))
        );
        await browser.pause(500);
      }
