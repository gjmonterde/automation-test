...
export default function suite() {
  it("Fj_TestScheme_63-1_0007_step_25: The 保証審査詳細 warranty examination details status is「承認待ち」", async () => {
    ...
    await $(
      "//label[contains(.,'" +
        test_data.testData.result_based_interest_lbl +
        "')]"
    ).$(function () {
      this.scrollIntoView({
        behavior: "smooth",
    ...
    await $(
      "//input[@id=//label[normalize-space(.)='" +
        test_data.testData.result_based_interest_lbl +
        "']/@for]"
    ).setValue(test_data.testData.result_based_interest_val);
    await browser.pause(3000);
