...
export default function suite() {
  it("Fj_TestScheme_63-1_0004_step_35: Executive confirmation results and fixed comments can be updated", async () => {
    ...
    await $(
      "//label[contains(.,'" + test_data.testData.standard_comment_lbl + "')]"
    ).$(function () {
      this.scrollIntoView({
        behavior: "smooth",
        block: "center",
    ...
    await $(
      "//input[@id=//label[normalize-space(.)='" +
        test_data.testData.standard_comment_lbl +
        "']/@for]"
    ).setValue(test_data.testData.standard_comment_value);
    await browser.pause(3000);
