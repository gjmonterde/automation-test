...
export default function suite() {
  it("Fj_TestScheme_30_0010_step_35: Create new 返済用銀行口座 record where 返済用フラグ = TRUE", async () => {
    ...
    const fininstname_lbl = await dialog
      ...
      .$(
        ".//label[contains(.,'" + test_data.testData.ba_bank_name_label + "')]"
      );
    ...
    const deposittype2 = await dialog
      .$(
        ".//records-record-layout-item[@field-label='" +
          test_data.testData.new_deposit_type_ordinary_bank_label +
          "']"
      )
      .$(
        ".//label[contains(.,'" +
    ...
    const deptype2val = await dialog
      ...
      .$(
        ".//lightning-base-combobox-item/span[contains(., '" +
          test_data.testData.new_deposit_type_ordinary_bank_value +
          "')]"
      );
