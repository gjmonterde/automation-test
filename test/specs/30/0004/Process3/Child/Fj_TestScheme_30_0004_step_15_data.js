...
export default async function suite() {
  it('Fj_TestScheme_30_0004_step_15_data: 顧客AMLチェック連携 Customer AML check collaboration for 銀行審査  Bank examination should be 「連携済み」"Collaborated".', async () => {
    ...
    const edit = await $(
      ".//button[@title='" + test_data.testData.ini3_linkage_edit_btn + "']"
    );
    ...
    // 審査情報連携ステータス
    await $(
      "//label[contains(.,'" + test_data.testData.linkage_status1 + "')]"
    ).$(function () {
      this.scrollIntoView({
        behavior: "smooth",
        block: "center",
    ...
    await $(".//div[@id='" + comboboxid1 + "']")
      .$("span=" + test_data.testData.linkage1)
      .click();
    await browser.pause(1000);

    // スコアリング情報連携ステータス
    await $(
      "//label[contains(.,'" + test_data.testData.linkage_status2 + "')]"
    ).$(function () {
      this.scrollIntoView({
        behavior: "smooth",
        block: "center",
    ...
    await $(".//div[@id='" + comboboxid2 + "']")
      .$("span=" + test_data.testData.linkage2)
      .click();
    await browser.pause(1000);

    // AML等チェック連携ステータス
    await $(
      "//label[contains(.,'" + test_data.testData.linkage_status3 + "')]"
    ).$(function () {
      this.scrollIntoView({
        behavior: "smooth",
        block: "center",
    ...
    await $(".//div[@id='" + comboboxid3 + "']")
      .$("span=" + test_data.testData.linkage3)
      .click();
    await browser.pause(1000);

    // 顧客AMLチェック連携ステータス
    await $(
      "//label[contains(.,'" + test_data.testData.federation_status + "')]"
    ).$(function () {
      this.scrollIntoView({
        behavior: "smooth",
        block: "center",
    ...
    await $(".//div[@id='" + comboboxid4 + "']")
      .$("span=" + test_data.testData.linkage4)
      .click();
